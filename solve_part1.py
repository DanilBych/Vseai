...
print(f"Avg |heating_rate| no failure: {avg_hr_no_failure:.6f}")
...
print(f"Failures with valid hr_3_before: {len(failures_valid)} out of {len(failures)}")

avg_hr_before_failure = failures_valid['hr_3_before'].abs().mean()
print(f"Avg |heating_rate| 3 orbits before failure: {avg_hr_before_failure:.6f}")
...
for col in model_cols:
    ...
    print(f"Model {model_num}: FPR={fpr:.4f}")
...
for lag in lags:
    if correlations_by_lag[lag]:
        ...
        print(f"Lag {lag}: avg corr = {avg_correlations[lag]:.6f}")
...
print(f"Partial corr 1: {partial_corr1:.6f}")
...
print(f"Partial corr 2: {partial_corr2:.6f}")
...
# Вопрос 7 - еще более оптимизированный с использованием numba
print("\n=== Вопрос 7 ===")
...
print("\n=== Вопросы 1-9 завершены ===")
print(f"Q1: {','.join(map(str, result_q1))}")
